...
// 加载所需模块
layui.use(['form','layer', 'laytpl', 'page_table', 'laydate', 'jacommon', 'tab','common'], function(){
	//  操作对象
	var form = layui.form,
		layer = layui.layer , 
		laytpl = layui.laytpl,
...
//		"dom": '<"top">rt<"bottom"flp><"clear">',
        ...
        "bLengthChange":true,       //是否显示每页多少条记录（没用？？？？）
	...
	$(document).on('click','#btn-delete-all', function(){
		...
		if(_val == null || _val == ''){
			...
			var startTime = nowTime.format("yyyy-MM-dd");

			if (_val != 1)  {
				startTime = nowTime.setDate(nowTime.getDate() - _val).format("yyyy-MM-dd");
			}
			...
			jacommon.confirm('确认删除' + _txt + '的登录日志吗？', function() {
				var postUrl = WEB_ROOT + '/admin/loginlog/delete?s=' + new Date().getTime();
